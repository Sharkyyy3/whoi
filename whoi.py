...
def test_multiprocessing(session_api,send, chunks, send_sticker_id, send_q1):


        print('Старт процесса0')
        p_func1 = Process(target=kuk, args=(send, chunks, send_sticker_id, send_q1))
        p_func1.start()
        print('Старт процесса1')
        p_func1.join()
        print('Рассылка завершенна!')
        #send_message(session_api, peer_id=106545709, tit='✦N.R.X✦',
        #             message='Рассылка завершенна!')
